#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# CareerOS — OpenShift deploy script
...
echo -e "\n${BOLD}${CYAN}CareerOS — Deploying to OpenShift${RESET}\n"
...
if [ -f config.json ]; then
  oc create configmap careeros-scraper-config \
...
echo -e "Get app URL:   oc get route careeros-app -n $NS"
echo -e "Trigger scrape: oc create job --from=cronjob/careeros-scraper manual-scrape -n $NS\n"
